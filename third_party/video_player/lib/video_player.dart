...
class VideoPlayerController extends ValueNotifier<VideoPlayerValue> {
    ...
    this.positionUpdateInterval = const Duration(milliseconds: 100),
       ...
       assert(
         positionUpdateInterval == null ||
             positionUpdateInterval > Duration.zero,
       ),
  ...
  /// How often playback position is sampled while the player is running.
  ///
  /// Callers that do not render a fine-grained seek position can use a longer
  /// interval to reduce platform-channel traffic and listener notifications.
  /// A null value disables position polling. Playback events, buffering,
  /// completion, and video-size updates continue to arrive normally.
  ...
  /// Whether position polling currently owns an active timer.
  @visibleForTesting
  bool get hasActivePositionTimer => _timer?.isActive ?? false;
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      final creatingCompleter = _creatingCompleter;
      if (creatingCompleter != null && !creatingCompleter.isCompleted) {
        creatingCompleter.complete();
      }
    }

    if (_isDisposed) {
      await _disposeCreatedPlayer();
      return;
    }
    ...
    void errorListener(Object error, StackTrace stackTrace) {
      ...
      _timer?.cancel();
      if (!initializingCompleter.isCompleted) {
        initializingCompleter.completeError(error, stackTrace);
      }
    }
  ...
  Future<void> _disposeResources() {
    final creatingCompleter = _creatingCompleter;
    if (creatingCompleter != null && !creatingCompleter.isCompleted) {
      return _waitForVideoPlayerCleanup(creatingCompleter.future).then((_) {
        return _disposeDetachedResources();
      });
    }
  ...
  Future<void> _disposeDetachedResources() {
    final subscription = _eventSubscription;
    _eventSubscription = null;
    final playerId = _takePlayerId();
    if (subscription != null) {
      try {
        ...
        // Detach the controller fields even if a platform stream is broken.
      }
    }

    return _disposeVideoPlayerResources(
  ...
  Future<void> _disposeCreatedPlayer() async {
    final playerId = _takePlayerId();
    if (playerId == kUninitializedPlayerId) {
      return;
    }

    try {
      await _waitForVideoPlayerCleanup(
        _videoPlayerPlatform.dispose(playerId),
      );
    } on Object {
      // Best-effort cleanup after disposal raced player creation.
    }
  }
  ...
  Future<void> _applyPlayPause() async {
    ...
    if (value.isPlaying) {
      ...
      _timer?.cancel();
      final interval = positionUpdateInterval;
      if (interval != null) {
        _timer = Timer.periodic(interval, (
          Timer timer,
        ) {
          if (_isDisposed || _isPollingPosition) {
            return;
          }
  ...
  Future<void> _pollPosition() async {
    try {
      ...
      // A failed position sample must not terminate periodic polling. The
      // platform event stream remains responsible for player-level errors.
    } finally {
...
Future<void> _waitForVideoPlayerCleanup(Future<dynamic> cleanup) async {
  await _VideoPlayerCleanupBarrier(cleanup).future;
}

Future<void> _disposeVideoPlayerResources({
  required StreamSubscription<dynamic>? subscription,
  required platform_interface.VideoPlayerPlatform platform,
  ...
  if (subscription != null) {
    try {
      ...
      // Continue with native player disposal.
    }
  }
  if (playerId != VideoPlayerController.kUninitializedPlayerId) {
    try {
      cleanup.add(platform.dispose(playerId));
    } on Object {
      // The controller is already detached from Flutter state.
    }
  }
...
const _videoPlayerCleanupTimeout = Duration(seconds: 3);

final class _VideoPlayerCleanupBarrier {
  _VideoPlayerCleanupBarrier(Future<dynamic> cleanup) {
    unawaited(
      cleanup.then<void>(
        (_) => _finish(),
    ...
    scheduleMicrotask(() {
      if (!_settled) {
        _timer = Timer(_videoPlayerCleanupTimeout, _finish);
      }
    });
  }
